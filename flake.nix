    ...
    flake-utils.lib.eachDefaultSystem (system:
        ...
        sources = {
          ocaml = nix-filter.lib {
            ...
            include = [
              ".ocamlformat"
              "dune-project"
              (nix-filter.lib.inDirectory "domains")
              (nix-filter.lib.inDirectory "frontend")
              (nix-filter.lib.inDirectory "iterator")
              (nix-filter.lib.inDirectory "libs")
              "analyzer.ml"
              "dune"
            ];
          };
      ...
      in
      {
        ...
        packages = {
          ...
          "${name}" = ocamlPackages.buildDunePackage {
            pname = "${name}";
            version = "0.1.0";
            duneVersion = "3";
            src = sources.ocaml;
        ...
        #
        checks = {
          # Run tests for the `${name}` package
          "${name}" =
            let
            ...
            self.packages.${system}.${name}.overrideAttrs
              (oldAttrs: {
                name = "check-${oldAttrs.name}";
                doCheck = true;
                buildPhase = patchDuneCommand oldAttrs.buildPhase;
                checkPhase = patchDuneCommand oldAttrs.checkPhase;
                # skip installation (this will be tested in the `${name}-app` check)
